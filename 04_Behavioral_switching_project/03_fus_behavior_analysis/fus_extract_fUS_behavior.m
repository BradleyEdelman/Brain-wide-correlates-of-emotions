function results_file = fus_extract_fUS_behavior(data_files, param)
...
if ~exist(results_file,'file') || exist(results_file,'file') && rewrite == 1
    ...
    I_bout_total = cell(1,size(data_files,1)-1); % time-locked fus data for each behavior for all sessions 
    I_bout_total_during = cell(1,size(data_files,1)-1); % time-locked behavior data for each behavior for all sessions
    
    I_bout_random_total = cell(1,size(data_files,1)-1); % time-locked random data for all sessions
    
    B_bout_total = cell(1,size(data_files,1)-1); % brain activity during each behavioral bout, independent of duration, for all sessions 
    B_bout_total_info = cell(1,size(data_files,1)-1); % bout info during each behavior for all sessions
    
    P_bout_total = cell(1,size(data_files,1)-1); % pupil diameter summed/averaged across each behavioral bout
    P_bout_total_during = cell(1,size(data_files,1)-1); % pupil diameter during each behavioral bout
    P_whole_recording = cell(size(data_files,1)-1,1); % pupil diameter for entire recording
    
    V_bout_total = cell(1,size(data_files,1)-1); % VBA data summed/averaged across each behavioral bout
    V_bout_total_during = cell(1,size(data_files,1)-1); % VBA data during each behavioral bout
    V_whole_recording = cell(size(data_files,1)-1,1); % VBA data for entire recording
   
    mouse_names = cell(size(data_files,1)-1,1); % mouse labels
    session_dates = cell(size(data_files,1)-1,1); % session labels
    L_bout_total = cell(1,size(data_files,1)-1); % full behavior labels (1-4) during each behavioral bout
    
    
    % For all sessions across all mice
    for i_session = 1:size(data_files,1)-1 
        i_session
        ...
        label_files_temp = {data_files{i_session + 1, [2:3 6]}};
        labels = fus_extract_behavior_labels(I_final, label_files_temp, param);
        ...
        I_bout_session_during = cell(4,1); % fus data during each behavioral bout, independent of duration, for current session 
        
        I_bout_random_session = cell(1,1); % time-locked random bout fus data
...
%             labels_behavior_interest = labels_behavior_interest';
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % RANDOM BOUT ANALYSIS
        
        % if random bout file already exists, load it and use pre-selected random onsets
        random_idx_file = [save_dir 'behavior_fus_random_bout_idx.mat'];
        if exist(random_idx_file,'file')
            
            load(random_idx_file)
            random_bouts_session = random_bouts_total(i_session,:);
            
        else
            % select random onsets
            random_bouts_session = datasample(1:size(I_final,2),param.behavior.random_bout);
            % check if window before or after onset exceeds data length
            out_of_range = sum(random_bouts_session - bout_window < 0 | random_bouts_session + bout_window > size(I_final,2));
            
            % iterate as needed
            while out_of_range ~= 0
                random_bouts_session = datasample(1:size(I_final,2),param.behavior.random_bout);
                out_of_range = sum(random_bouts_session - bout_window < 0 | random_bouts_session + bout_window > size(I_final,2));
            end
            
            % store for saving later
            random_bouts_total(i_session,:) = random_bouts_session;
            
        end
        
        % store random bout fus data
        for i_rand = 1:size(random_bouts_session,2)
            window_idx = random_bouts_session(i_rand) - bout_window + 1:random_bouts_session(i_rand) + bout_window; % window indexes
            I_bout_random_session{1}(:,:,i_rand) = I_final(:,window_idx); 
        end
        ...
        I_bout_random_total{i_session} = I_bout_random_session;
    ...
    %% Save data
%     fprintf('\n Saving %s\n', results_file)
%     save(results_file, 'I_bout_total', 'I_bout_total_during', 'I_bout_random_total', 'P_bout_total',...
...
%         'new_name', 'new_seg', 'mouse_names', 'session_dates', '-v7.3');
%     fprintf('Finished \n')
    
    % ...or more concise
    fprintf('\n Saving %s\n', results_file)
    save(results_file, 'I_bout_total','I_bout_random_total', 'P_bout_total_during',...
        'V_bout_total_during', 'I_bout_total_during','P_bout_total', 'B_bout_total',...
        'B_bout_total_info', 'L_bout_total', 'V_bout_total',...
        'V_whole_recording', 'P_whole_recording', 'new_name', 'new_seg',...
        'mouse_names', 'session_dates', '-v7.3');
    fprintf('Finished \n')
    
    % also save random idx file
    fprintf('\n Saving %s\n', random_idx_file)
    save(random_idx_file, 'random_bouts_total');
    fprintf('Finished \n')
