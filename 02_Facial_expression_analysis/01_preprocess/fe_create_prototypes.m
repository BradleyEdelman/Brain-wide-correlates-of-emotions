function fe_create_prototypes(data, param)
...
for i_mouse = 1:size(data.mouse,2)
    ...
    for i_fold = 1:size(data.idxProto,2)
        ...
        iparts = strsplit(fold_analysis{data.idxProto(i_mouse,i_fold)},'\');
        save_file = [proto_fold 'PROTO_' iparts{end-2} '_2021112.mat'];
        if ~exist(save_file,'file') || rewrite == 1
        
            % load hogs
            hog_file = [fold_analysis{data.idxProto(i_mouse,i_fold)} 'hogs.mat'];
            ...
            DLC_file = [fold_analysis{data.idxProto(i_mouse,i_fold)} 'invalid_indices_2paw.mat'];
...
%             idxStim = idxStim(:,2);
            ...
            files = dir(fold{data.idxProto(i_mouse,i_fold)});
            filename = {files.name};
            filename = filename(cellfun(@(s)~isempty(regexp(s,'.jpg')),filename));
            num_img = 20; clear img_proto img_neut
            for i_fr = 1:num_img
                img_proto(:,:,i_fr) = imread([fold{data.idxProto(i_mouse,i_fold)} filename{I(i_fr)}]);
                img_neut(:,:,i_fr) = imread([fold{data.idxProto(i_mouse,i_fold)} filename{I(end-num_img+i_fr)}]);
            end
