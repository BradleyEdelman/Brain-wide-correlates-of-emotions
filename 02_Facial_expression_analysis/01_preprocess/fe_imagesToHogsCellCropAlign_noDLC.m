function hog = fe_imagesToHogsCellCropAlign_noDLC(imageFolder, referenceFolder, param, cropInfo, varargin)
...
% only if files are present in both the reference and current image folder
if ~isempty(filename) && ~isempty(filenameR)
    
    %%%%%%%% when picking neutral frames, look at motion energy to identify
    %%%%%%%% frames with little changing (indicates resting state)

    L = min([100 min([size(filenameR,2) size(filename,2)])]);
    % take L random images from each image stack for registration
    ridxRgood = randperm(3500); 
    ridxIgood = randperm(3500);
    for i_iter = 1:L
