function fe_proto_normalize_newstim(data, param)
...
for i_mouse = 1:size(CC_proto,2)
    ...
    for i_proto = 1:size(CC_proto_mouse,1)
        MM(i_proto, :, i_mouse) = [nanmax(CC_proto_mouse(i_proto,idxBase)) nanmax(CC_proto_mouse(i_proto,idxStim))];
    end
...
for i_mouse = 1:size(CC_new,3)
    ...
    for i_stim = 1:size(CC_new,2)
        if ~isempty(CC_new_tmp{1,i_stim})
            ...
            plot_areaerrorbar(tmp',[], F); ylim([0 .65])
...
figure(100); clf
F = [100, 1, 3, 1];
tmp = cat(2,new_trace{1,idx,:});
plot_areaerrorbar(tmp', [], F); ylim([0 3])
F = [100, 1, 3, 2];
tmp = cat(2,new_trace{2,idx,:});
plot_areaerrorbar(tmp', [], F); ylim([0 3])
subplot(1,3,3);
tmp = cat(1,new_trace{3,idx,:}); ylim([0 3])
boxplot(tmp);
